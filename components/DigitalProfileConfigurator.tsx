...
export const DigitalProfileConfigurator = () => {
  ...
  const renderTemplate = () => {
    switch (profile.template) {
      ...
      // 1. STYLE LINKEDIN (Bannière, Photo décalée)
      case 'linkedin':
        return (
          <div className="h-full bg-[#f3f2ef] text-black overflow-y-auto font-sans">
            <div className="h-32 bg-[url('https://images.unsplash.com/photo-1579546929518-9e396f3cc809?w=800&q=80')] bg-cover bg-center"></div>
            <div className="px-6 pb-8 relative">
              <div className="w-32 h-32 rounded-full border-4 border-white bg-white absolute -top-16 left-4 overflow-hidden shadow-sm">
                <img src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=800&q=80" className="w-full h-full object-cover" />
              </div>
              <div className="mt-16 mb-6">
                <h2 className="text-2xl font-bold text-gray-900 leading-tight">{profile.name}</h2>
                <p className="text-gray-600 text-sm mb-1">{profile.title}</p>
                <p className="text-gray-500 text-xs flex items-center gap-1">
                  <MapPin className="w-3 h-3" /> {profile.location}
                </p>
              </div>
              <div className="flex gap-2 mb-6">
                <button className="flex-1 bg-[#0a66c2] text-white py-1.5 rounded-full font-bold text-sm">Se connecter</button>
                <button className="flex-1 border border-gray-400 text-gray-600 py-1.5 rounded-full font-bold text-sm">Message</button>
              </div>
